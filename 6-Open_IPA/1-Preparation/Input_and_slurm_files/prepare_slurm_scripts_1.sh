...
for name in min1 min2 min3 min4 min5 min6 heat; do
  mpirun -n 12 \$AMBERHOME/bin/pmemd.MPI -O -i \${name}.i -o open_ipa_${i}_\${name}.mdout -p open_ipa_HMR.parm7 -c open_ipa_${i}_\${old}.rst7 -ref open_ipa_${i}_ini.rst7 -x open_ipa_${i}_\${name}.nc -r open_ipa_${i}_\${name}.rst7 -inf open_ipa_${i}_\${name}.mdinfo
...
trajout open_ipa_${i}_\${name}.rst7 ncrestart
run
EOF
cpptraj < image.in
rm image.in
...
" >  run.slurm
  ...
  cp *.i ../../2-MD1/${i}
  cp ../open_ipa_10_HMR.parm7 ../../2-MD1/${i}/open_ipa_HMR.parm7
  cp ../open_ipa_10_HMR.rst7 ../../2-MD1/${i}/open_ipa_${i}_ini.rst7
done
