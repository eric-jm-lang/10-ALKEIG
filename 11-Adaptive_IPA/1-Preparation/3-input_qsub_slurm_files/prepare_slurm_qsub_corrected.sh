...
for name in eq1 eq2 eq3 eq4 eq5 eq6; do
...
for name in eq7 md1; do

\$AMBERHOME/bin/pmemd.cuda -O -i \${name}.i -o adapt1_ipa_${i}_\${name}.mdout -p adapt1_ipa_${i}_wat_HMR.parm7 -c adapt1_ipa_${i}_\${old}.rst7 -ref adapt1_ipa_${i}_wat.rst7 -x adapt1_ipa_${i}_\${name}.nc -r adapt1_ipa_${i}_\${name}.rst7 -inf adapt1_ipa_${i}_\${name}.mdinfo

cp adapt1_ipa_${i}_\${name}.rst7 adapt1_ipa_${i}_\${name}.rst7.ORG
mv adapt1_ipa_${i}_\${name}.rst7 TEMP_adapt1_ipa_${i}_\${name}.rst7

cat > image.in <<EOF
parm adapt1_ipa_${i}_wat_HMR.parm7
trajin TEMP_adapt1_ipa_${i}_\${name}.rst7
center :1-192 mass origin
image familiar com :1-192
trajout adapt1_ipa_${i}_\${name}.rst7 restart
run
EOF
cpptraj < image.in
rm image.in
